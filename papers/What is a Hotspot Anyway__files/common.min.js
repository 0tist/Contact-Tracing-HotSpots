var HANDJS=HANDJS||{};(function(){if(window.PointerEvent){return}if(!Array.prototype.indexOf){Array.prototype.indexOf=function(F){var G=Object(this);var D=G.length>>>0;if(D===0){return -1}var H=0;if(arguments.length>0){H=Number(arguments[1]);if(H!=H){H=0}else{if(H!=0&&H!=Infinity&&H!=-Infinity){H=(H>0||-1)*Math.floor(Math.abs(H))}}}if(H>=D){return -1}var E=H>=0?H:Math.max(D-Math.abs(H),0);for(;E<D;E++){if(E in G&&G[E]===F){return E}}return -1}}if(!Array.prototype.forEach){Array.prototype.forEach=function(F,D){if(!this||!(F instanceof Function)){throw new TypeError()}for(var E=0;E<this.length;E++){F.call(D,this[E],E,this)}}}if(!String.prototype.trim){String.prototype.trim=function(){return this.replace(/^\s+|\s+$/,"")}}var C=["pointerdown","pointerup","pointermove","pointerover","pointerout","pointercancel","pointerenter","pointerleave"];var h=["PointerDown","PointerUp","PointerMove","PointerOver","PointerOut","PointerCancel","PointerEnter","PointerLeave"];var o="touch";var r="pen";var t="mouse";var j={};var e=function(D){while(D&&!D.handjs_forcePreventDefault){D=D.parentNode}return !!D||window.handjs_forcePreventDefault};var c=function(G,E,I,K,D){var F;if(document.createEvent){F=document.createEvent("MouseEvents");F.initMouseEvent(E,I,true,window,1,G.screenX,G.screenY,G.clientX,G.clientY,G.ctrlKey,G.altKey,G.shiftKey,G.metaKey,G.button,D||G.relatedTarget)}else{F=document.createEventObject();F.screenX=G.screenX;F.screenY=G.screenY;F.clientX=G.clientX;F.clientY=G.clientY;F.ctrlKey=G.ctrlKey;F.altKey=G.altKey;F.shiftKey=G.shiftKey;F.metaKey=G.metaKey;F.button=G.button;F.relatedTarget=D||G.relatedTarget}if(F.offsetX===undefined){if(G.offsetX!==undefined){if(Object&&Object.defineProperty!==undefined){Object.defineProperty(F,"offsetX",{writable:true});Object.defineProperty(F,"offsetY",{writable:true})}F.offsetX=G.offsetX;F.offsetY=G.offsetY}else{if(Object&&Object.defineProperty!==undefined){Object.defineProperty(F,"offsetX",{get:function(){if(this.currentTarget&&this.currentTarget.offsetLeft){return G.clientX-this.currentTarget.offsetLeft}return G.clientX}});Object.defineProperty(F,"offsetY",{get:function(){if(this.currentTarget&&this.currentTarget.offsetTop){return G.clientY-this.currentTarget.offsetTop}return G.clientY}})}else{if(G.layerX!==undefined){F.offsetX=G.layerX-G.currentTarget.offsetLeft;F.offsetY=G.layerY-G.currentTarget.offsetTop}}}}if(G.isPrimary!==undefined){F.isPrimary=G.isPrimary}else{F.isPrimary=true}if(G.pressure){F.pressure=G.pressure}else{var H=0;if(G.which!==undefined){H=G.which}else{if(G.button!==undefined){H=G.button}}F.pressure=(H==0)?0:0.5}if(G.rotation){F.rotation=G.rotation}else{F.rotation=0}if(G.hwTimestamp){F.hwTimestamp=G.hwTimestamp}else{F.hwTimestamp=0}if(G.tiltX){F.tiltX=G.tiltX}else{F.tiltX=0}if(G.tiltY){F.tiltY=G.tiltY}else{F.tiltY=0}if(G.height){F.height=G.height}else{F.height=0}if(G.width){F.width=G.width}else{F.width=0}F.preventDefault=function(){if(G.preventDefault!==undefined){G.preventDefault()}};if(F.stopPropagation!==undefined){var J=F.stopPropagation;F.stopPropagation=function(){if(G.stopPropagation!==undefined){G.stopPropagation()}J.call(this)}}F.pointerId=G.pointerId;F.pointerType=G.pointerType;switch(F.pointerType){case 2:F.pointerType=o;break;case 3:F.pointerType=r;break;case 4:F.pointerType=t;break}if(K){K.dispatchEvent(F)}else{if(G.target){G.target.dispatchEvent(F)}else{G.srcElement.fireEvent("on"+v(E),F)}}};var k=function(E,F,G,H,D){E.pointerId=1;E.pointerType=t;c(E,F,G,H,D)};var p=function(F,G,I,E,H,D){var J=G.identifier+2;G.pointerId=J;G.pointerType=o;G.currentTarget=I;if(E.preventDefault!==undefined){G.preventDefault=function(){E.preventDefault()}}c(G,F,H,I,D)};var s=function(E,D){return E.__handjsGlobalRegisteredEvents&&E.__handjsGlobalRegisteredEvents[D]};var a=function(E,D){while(E&&!s(E,D)){E=E.parentNode}if(E){return E}else{if(s(window,D)){return window}}};var d=function(F,G,I,E,H,D){if(a(I,F)){p(F,G,I,E,H,D)}};var v=function(D){return D.toLowerCase().replace("pointer","mouse")};var y=function(F,D){var E=C.indexOf(D);var G=F+h[E];return G};var B=function(G,E,F,D){if(G.__handjsRegisteredEvents===undefined){G.__handjsRegisteredEvents=[]}if(D){if(G.__handjsRegisteredEvents[E]!==undefined){G.__handjsRegisteredEvents[E]++;return}G.__handjsRegisteredEvents[E]=1;G.addEventListener(E,F,false)}else{if(G.__handjsRegisteredEvents.indexOf(E)!==-1){G.__handjsRegisteredEvents[E]--;if(G.__handjsRegisteredEvents[E]!=0){return}}G.removeEventListener(E,F);G.__handjsRegisteredEvents[E]=0}};var f=function(H,E,F){if(!H.__handjsGlobalRegisteredEvents){H.__handjsGlobalRegisteredEvents=[]}if(F){if(H.__handjsGlobalRegisteredEvents[E]!==undefined){H.__handjsGlobalRegisteredEvents[E]++;return}H.__handjsGlobalRegisteredEvents[E]=1}else{if(H.__handjsGlobalRegisteredEvents[E]!==undefined){H.__handjsGlobalRegisteredEvents[E]--;if(H.__handjsGlobalRegisteredEvents[E]<0){H.__handjsGlobalRegisteredEvents[E]=0}}}var I;var D;if(window.MSPointerEvent){I=function(J){return y("MS",J)};D=c}else{I=v;D=k}switch(E){case"pointerenter":case"pointerleave":var G=I(E);if(H["on"+G.toLowerCase()]!==undefined){B(H,G,function(J){D(J,E)},F)}break}};var q=function(D){var F=D.prototype?D.prototype.addEventListener:D.addEventListener;var E=function(H,I,G){if(C.indexOf(H)!=-1){f(this,H,true)}if(F===undefined){this.attachEvent("on"+v(H),I)}else{F.call(this,H,I,G)}};if(D.prototype){D.prototype.addEventListener=E}else{D.addEventListener=E}};var n=function(D){var F=D.prototype?D.prototype.removeEventListener:D.removeEventListener;var E=function(H,I,G){if(C.indexOf(H)!=-1){f(this,H,false)}if(F===undefined){this.detachEvent(v(H),I)}else{F.call(this,H,I,G)}};if(D.prototype){D.prototype.removeEventListener=E}else{D.removeEventListener=E}};q(window);q(window.HTMLElement||window.Element);q(document);q(HTMLBodyElement);q(HTMLDivElement);q(HTMLImageElement);q(HTMLUListElement);q(HTMLAnchorElement);q(HTMLLIElement);q(HTMLTableElement);if(window.HTMLSpanElement){q(HTMLSpanElement)}if(window.HTMLCanvasElement){q(HTMLCanvasElement)}if(window.SVGElement){q(SVGElement)}n(window);n(window.HTMLElement||window.Element);n(document);n(HTMLBodyElement);n(HTMLDivElement);n(HTMLImageElement);n(HTMLUListElement);n(HTMLAnchorElement);n(HTMLLIElement);n(HTMLTableElement);if(window.HTMLSpanElement){n(HTMLSpanElement)}if(window.HTMLCanvasElement){n(HTMLCanvasElement)}if(window.SVGElement){n(SVGElement)}var b=false;var l=-1;function x(){b=true;clearTimeout(l);l=setTimeout(function(){b=false},700)}function m(E){var D=[];if(E){D.unshift(E);while(E.parentNode){D.unshift(E.parentNode);E=E.parentNode}}return D}function z(G,E){var F=m(G);var D=m(E);var H=null;while(F.length>0&&F[0]==D.shift()){H=F.shift()}return H}function u(G,D,I){var H=z(G,D);var F=G;var E=[];while(F&&F!=H){if(s(F,"pointerenter")){E.push(F)}F=F.parentNode}while(E.length>0){I(E.pop())}}function g(F,D,H){var G=z(F,D);var E=F;while(E&&E!=G){if(s(E,"pointerleave")){H(E)}E=E.parentNode}}function A(E,D){["pointerdown","pointermove","pointerup","pointerover","pointerout"].forEach(function(F){window.addEventListener(E(F),function(G){if(!b&&a(G.target,F)){D(G,F,true)}})});if(window["on"+E("pointerenter").toLowerCase()]===undefined){window.addEventListener(E("pointerover"),function(G){if(b){return}var F=a(G.target,"pointerenter");if(!F||F===window){return}else{if(!F.contains(G.relatedTarget)){u(F,G.relatedTarget,function(H){D(G,"pointerenter",false,H,G.relatedTarget)})}}})}if(window["on"+E("pointerleave").toLowerCase()]===undefined){window.addEventListener(E("pointerout"),function(G){if(b){return}var F=a(G.target,"pointerleave");if(!F||F===window){return}else{if(!F.contains(G.relatedTarget)){g(F,G.relatedTarget,function(H){D(G,"pointerleave",false,H,G.relatedTarget)})}}})}}(function(){if(window.MSPointerEvent){A(function(D){return y("MS",D)},c)}else{A(v,k);if(window.ontouchstart!==undefined){window.addEventListener("touchstart",function(D){for(var E=0;E<D.changedTouches.length;++E){var F=D.changedTouches[E];j[F.identifier]=F.target;d("pointerover",F,F.target,D,true);u(F.target,null,function(G){p("pointerenter",F,G,D,false)});d("pointerdown",F,F.target,D,true)}x()});window.addEventListener("touchend",function(D){for(var E=0;E<D.changedTouches.length;++E){var F=D.changedTouches[E];var G=j[F.identifier];d("pointerup",F,G,D,true);d("pointerout",F,G,D,true);g(G,null,function(H){p("pointerleave",F,H,D,false)})}x()});window.addEventListener("touchmove",function(D){for(var E=0;E<D.changedTouches.length;++E){var F=D.changedTouches[E];var H=document.elementFromPoint(F.clientX,F.clientY);var G=j[F.identifier];if(G&&e(G)===true){D.preventDefault()}d("pointermove",F,G,D,true);if(G===H){continue}if(G){d("pointerout",F,G,D,true,H);if(!G.contains(H)){g(G,H,function(I){p("pointerleave",F,I,D,false,H)})}}if(H){d("pointerover",F,H,D,true,G);if(!H.contains(G)){u(H,G,function(I){p("pointerenter",F,I,D,false,G)})}}j[F.identifier]=H}x()});window.addEventListener("touchcancel",function(D){for(var E=0;E<D.changedTouches.length;++E){var F=D.changedTouches[E];d("pointercancel",F,j[F.identifier],D,true)}})}}})();if(navigator.pointerEnabled===undefined){navigator.pointerEnabled=true;if(navigator.msPointerEnabled){navigator.maxTouchPoints=navigator.msMaxTouchPoints}}})();
/*
